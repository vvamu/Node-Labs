...
module.exports = () => {
  ...
  router
    ...
    .get((req, res, next) => {
      ...
      var a =  req.query.favoritePizzaName;
      if (Object.keys(req.query).length === 0) {
        turtleController.getAllTurtles(req, res, next);
      } else {
        if(a != null){
          turtleController.getTurtlesByFavoritePizzaName(req,res,next);
        }
        next();
      }
    })
    .get(turtleController.getTurtleById)
  ...
  router.route("/secondFavoritePizzaBind").put(turtleController.bindSecondFavoritePizza);
  router.route("/weaponBind").put(turtleController.bindWeapon);
  
  router.route("/favoritePizzaUnbind").delete(turtleController.unbindFavoritePizza);
  router.route("/secondFavoritePizzaUnbind").delete(turtleController.unbindSecondFavoritePizza);
